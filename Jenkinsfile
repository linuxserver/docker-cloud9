    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=nodejs -v ${TEMPDIR}:/ansible/readme linuxserver/doc-builder:latest
              if [ "$(md5sum ${TEMPDIR}/${LS_REPO}/Jenkinsfile | awk '{ print $1 }')" != "$(md5sum Jenkinsfile | awk '{ print $1 }')" ] || \
                ...
                git --git-dir ${TEMPDIR}/repo/${LS_REPO}/.git checkout -f nodejs
      ...
      steps {
        withCredentials([
          ...
          sh '''#! /bin/bash
             echo $DOCKERPASS | docker login -u $DOCKERUSER --password-stdin
             '''
          sh "docker tag ${IMAGE}:${META_TAG} ${IMAGE}:nodejs"
          sh "docker push ${IMAGE}:nodejs"
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:amd64-${META_TAG} ${IMAGE}:amd64-nodejs"
          sh "docker tag ${IMAGE}:arm32v7-${META_TAG} ${IMAGE}:arm32v7-nodejs"
          sh "docker tag ${IMAGE}:arm64v8-${META_TAG} ${IMAGE}:arm64v8-nodejs"
          sh "docker push ${IMAGE}:amd64-${META_TAG}"
          sh "docker push ${IMAGE}:arm32v7-${META_TAG}"
          sh "docker push ${IMAGE}:arm64v8-${META_TAG}"
          sh "docker push ${IMAGE}:amd64-nodejs"
          sh "docker push ${IMAGE}:arm32v7-nodejs"
          sh "docker push ${IMAGE}:arm64v8-nodejs"
          sh "docker manifest push --purge ${IMAGE}:nodejs || :"
          sh "docker manifest create ${IMAGE}:nodejs ${IMAGE}:amd64-nodejs ${IMAGE}:arm32v7-nodejs ${IMAGE}:arm64v8-nodejs"
          sh "docker manifest annotate ${IMAGE}:nodejs ${IMAGE}:arm32v7-nodejs --os linux --arch arm"
          sh "docker manifest annotate ${IMAGE}:nodejs ${IMAGE}:arm64v8-nodejs --os linux --arch arm64 --variant v8"
          ...
          sh '''docker rmi \
                ...
                ${IMAGE}:amd64-nodejs \
                ${IMAGE}:arm32v7-${META_TAG} \
                ${IMAGE}:arm32v7-nodejs \
                ${IMAGE}:arm64v8-${META_TAG} \
                ${IMAGE}:arm64v8-nodejs \
                lsiodev/buildcache:arm32v7-${COMMIT_SHA}-${BUILD_NUMBER} \
                lsiodev/buildcache:arm64v8-${COMMIT_SHA}-${BUILD_NUMBER} || :'''
        }
      }
