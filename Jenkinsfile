        ...
        withCredentials([
          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  docker tag ${GITHUBIMAGE}:amd64-${META_TAG} ${GITHUBIMAGE}:python
                  docker tag ${GITHUBIMAGE}:amd64-${META_TAG} ${GITHUBIMAGE}:${META_TAG}
                  docker tag ${GITHUBIMAGE}:arm32v7-${META_TAG} ${GITHUBIMAGE}:arm32v7-python
                  docker tag ${GITHUBIMAGE}:arm64v8-${META_TAG} ${GITHUBIMAGE}:arm64v8-python
                  docker push ${GITHUBIMAGE}:amd64-${META_TAG}
                  docker push ${GITHUBIMAGE}:arm32v7-${META_TAG}
                  docker push ${GITHUBIMAGE}:arm64v8-${META_TAG}
                  docker push ${GITHUBIMAGE}:python
                  docker push ${GITHUBIMAGE}:${META_TAG}
                  docker push ${GITHUBIMAGE}:arm32v7-python
                  docker push ${GITHUBIMAGE}:arm64v8-python
               '''
          }
          sh '''#! /bin/bash
                for DELETEIMAGE in "${GITHUBIMAGE}" "${GITLABIMAGE}" "${IMAGE}"; do
