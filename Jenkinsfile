    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=ruby -v ${TEMPDIR}:/ansible/readme linuxserver/doc-builder:latest
              if [ "$(md5sum ${TEMPDIR}/${LS_REPO}/Jenkinsfile | awk '{ print $1 }')" != "$(md5sum Jenkinsfile | awk '{ print $1 }')" ] || \
                ...
                git --git-dir ${TEMPDIR}/repo/${LS_REPO}/.git checkout -f ruby
      ...
      steps {
        withCredentials([
          ...
          sh '''#! /bin/bash
             echo $DOCKERPASS | docker login -u $DOCKERUSER --password-stdin
             '''
          sh "docker tag ${IMAGE}:${META_TAG} ${IMAGE}:ruby"
          sh "docker push ${IMAGE}:ruby"
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:amd64-${META_TAG} ${IMAGE}:amd64-ruby"
          sh "docker tag ${IMAGE}:arm32v7-${META_TAG} ${IMAGE}:arm32v7-ruby"
          sh "docker tag ${IMAGE}:arm64v8-${META_TAG} ${IMAGE}:arm64v8-ruby"
          sh "docker push ${IMAGE}:amd64-${META_TAG}"
          sh "docker push ${IMAGE}:arm32v7-${META_TAG}"
          sh "docker push ${IMAGE}:arm64v8-${META_TAG}"
          sh "docker push ${IMAGE}:amd64-ruby"
          sh "docker push ${IMAGE}:arm32v7-ruby"
          sh "docker push ${IMAGE}:arm64v8-ruby"
          sh "docker manifest push --purge ${IMAGE}:ruby || :"
          sh "docker manifest create ${IMAGE}:ruby ${IMAGE}:amd64-ruby ${IMAGE}:arm32v7-ruby ${IMAGE}:arm64v8-ruby"
          sh "docker manifest annotate ${IMAGE}:ruby ${IMAGE}:arm32v7-ruby --os linux --arch arm"
          sh "docker manifest annotate ${IMAGE}:ruby ${IMAGE}:arm64v8-ruby --os linux --arch arm64 --variant v8"
