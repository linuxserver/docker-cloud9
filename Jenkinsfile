    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=python -v ${TEMPDIR}:/ansible/readme linuxserver/doc-builder:latest
              if [ "$(md5sum ${TEMPDIR}/${LS_REPO}/Jenkinsfile | awk '{ print $1 }')" != "$(md5sum Jenkinsfile | awk '{ print $1 }')" ] || \
                ...
                git --git-dir ${TEMPDIR}/repo/${LS_REPO}/.git checkout -f python
      ...
      steps {
        withCredentials([
          ...
          sh '''#! /bin/bash
             echo $DOCKERPASS | docker login -u $DOCKERUSER --password-stdin
             '''
          sh "docker tag ${IMAGE}:${META_TAG} ${IMAGE}:python"
          sh "docker push ${IMAGE}:python"
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:amd64-${META_TAG} ${IMAGE}:amd64-python"
          sh "docker tag ${IMAGE}:arm32v7-${META_TAG} ${IMAGE}:arm32v7-python"
          sh "docker tag ${IMAGE}:arm64v8-${META_TAG} ${IMAGE}:arm64v8-python"
          sh "docker push ${IMAGE}:amd64-${META_TAG}"
          sh "docker push ${IMAGE}:arm32v7-${META_TAG}"
          sh "docker push ${IMAGE}:arm64v8-${META_TAG}"
          sh "docker push ${IMAGE}:amd64-python"
          sh "docker push ${IMAGE}:arm32v7-python"
          sh "docker push ${IMAGE}:arm64v8-python"
          sh "docker manifest push --purge ${IMAGE}:python || :"
          sh "docker manifest create ${IMAGE}:python ${IMAGE}:amd64-python ${IMAGE}:arm32v7-python ${IMAGE}:arm64v8-python"
          sh "docker manifest annotate ${IMAGE}:python ${IMAGE}:arm32v7-python --os linux --arch arm"
          sh "docker manifest annotate ${IMAGE}:python ${IMAGE}:arm64v8-python --os linux --arch arm64 --variant v8"
